    ...
    <div className="min-h-screen bg-stone-50 text-stone-900 overflow-hidden">
      ...
      <section className="relative h-screen overflow-hidden">
        ...
        <div className="fixed bottom-6 right-6 z-40">
          <button className="bg-stone-900 text-white px-6 py-3 rounded-full shadow-lg hover:bg-stone-800 transition-all duration-300 flex items-center gap-3">
      ...
      {/* Meet Your AI Workers Section */}
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold mb-4 text-stone-900">
              Meet Your AI
              <span className="text-amber-800"> Workers</span>
            </h2>
            <p className="text-lg text-stone-700 max-w-3xl mx-auto leading-relaxed">
              Our AI workers don't just analyze data – they transform your business. With 24/7 operations, 
              multilingual capabilities, and human-like intelligence, they're revolutionizing ecommerce operations.
            </p>
          </div>
          ...
          <div className="grid lg:grid-cols-3 gap-6 xl:gap-8">
            {[
              {
                name: 'Dhana',
                role: 'AI Pricing Strategist',
                description: 'Dhana transforms your pricing into profit. She analyzes competitor data, market trends, and customer behavior to optimize your margins in real-time.',
                features: ['Dynamic pricing optimization', 'Competitor price monitoring', 'Profit margin analysis', 'Market trend prediction'],
                avatar: '👩‍💼',
                color: 'from-amber-600 to-orange-700',
                bgColor: 'from-amber-50 to-orange-50',
                stats: { metric: '35%', label: 'Average profit increase' }
              },
              {
                name: 'Vivek',
                role: 'AI Sentiment Analyst',
                description: 'Vivek reads between the lines of every customer review. He understands emotions, identifies issues, and helps you improve customer satisfaction.',
                features: ['Review sentiment analysis', 'Customer feedback insights', 'Product performance tracking', 'Satisfaction trend monitoring'],
                avatar: '👨‍💻',
                color: 'from-stone-700 to-stone-800',
                bgColor: 'from-stone-50 to-stone-100',
                stats: { metric: '40%', label: 'Satisfaction improvement' }
              },
              {
                name: 'Agrim',
                role: 'AI Demand Forecaster',
                description: 'Agrim predicts the future of your inventory. He analyzes patterns, seasonal trends, and market signals to optimize your stock levels.',
                features: ['Demand forecasting', 'Inventory optimization', 'Seasonal trend analysis', 'Stock level recommendations'],
                avatar: '👨‍🔬',
                color: 'from-emerald-600 to-teal-700',
                bgColor: 'from-emerald-50 to-teal-50',
                stats: { metric: '25%', label: 'Inventory cost reduction' }
              }
            ].map((worker, index) => (
              <div key={index} className="group">
                <div className="bg-white rounded-2xl p-4 lg:p-6 border border-stone-200 hover:border-stone-300 transition-all duration-500 hover:shadow-xl hover:scale-105 shadow-lg h-full flex flex-col">
                  {/* Worker Avatar and Info */}
                  <div className="flex items-center mb-4">
                    <div className="text-2xl lg:text-3xl mr-3">{worker.avatar}</div>
                    <div className="min-w-0 flex-1">
                      <h3 className="text-lg lg:text-xl font-bold text-stone-900 mb-1 truncate">{worker.name}</h3>
                      <p className={`text-xs lg:text-sm bg-gradient-to-r ${worker.color} bg-clip-text text-transparent font-semibold`}>
                        {worker.role}
                      </p>
                    </div>
                  </div>

                  <p className="text-stone-700 mb-4 leading-relaxed text-sm lg:text-base flex-1">{worker.description}</p>

                  {/* Features List */}
                  <ul className="space-y-2 mb-4">
                    {worker.features.map((feature, idx) => (
                      <li key={idx} className="flex items-start text-stone-600 text-xs lg:text-sm">
                        <div className={`w-1.5 h-1.5 lg:w-2 lg:h-2 bg-gradient-to-r ${worker.color} rounded-full mr-2 mt-1.5 flex-shrink-0`}></div>
                  ...
                  {/* Performance Stats */}
                  <div className={`bg-gradient-to-r ${worker.bgColor} rounded-xl p-3 text-center border border-stone-200 mb-4`}>
                    <div className={`text-xl lg:text-2xl font-bold bg-gradient-to-r ${worker.color} bg-clip-text text-transparent mb-1`}>
                      {worker.stats.metric}
                    </div>
                    <div className="text-stone-700 text-xs lg:text-sm">{worker.stats.label}</div>
                  </div>

                  {/* Hire Button */}
                  <button className={`w-full py-2 lg:py-3 bg-gradient-to-r ${worker.color} rounded-lg hover:shadow-lg transition-all duration-300 font-semibold text-white hover:scale-105 text-sm lg:text-base`}>
                    Deploy {worker.name}
                  </button>
                </div>
              </div>
      ...
      {/* CTA Section - 11x.ai Style */}
      <section className="relative py-20 bg-gradient-to-br from-stone-200 via-amber-100 to-stone-300 overflow-hidden">
        {/* Background Image */}
